...
documentacion — y la incorpora a la suite de requisitos/planificacion/build. Pensado
para el caso vibe-coding: alguien tuvo una idea, la prompteo, y hoy tiene un codebase
que funciona (en parte) pero nadie sabe con precision que hace, que falta, ni que
decisiones se tomaron en el camino.
...
4. **El estado honesto y las preguntas**: un reporte de estado por feature (que falta
   ...
   pantalla de reportes existe pero no muestra datos: ¿la terminamos, la sacamos, o
   era de otra idea?"). Tus respuestas se aplican a la reconstruccion.

## Por que importa el formato

Porque despues de comprender, **todo lo demas ya funciona**:

```
/comprender                          la app entra a la suite
...
→ simbolo del LEL → `archivo:linea`; y cada corrida de comprension queda registrada en
el changelog (`REC-xxx`).
...
fuente. Escribe en `.dev/recovery/` (inventario, mapa de comportamiento, reporte de
estado, cuestionario) y `.dev/requirements/` (la linea de base).

Tiene una pausa: el cuestionario del dueño. Responderlo afina la reconstruccion
(features que se confirman, se recortan o se descartan); tambien podes responderlo
mas tarde y re-correr `/comprender`.
...
recovery-pipeline/
  ...
  agents/
    ...
    behavior-extraction.md     que hace la app, con archivo:linea
  ...
  skills/recovery-pipeline/SKILL.md

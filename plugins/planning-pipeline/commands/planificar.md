...
3. Corre `plan-inspection` y su lazo de correccion hasta que el plan pase. Los rebotes
   van a `task-derivation`, `sprint-planning` o `parallel-planning` segun el defecto.
...
5. Al final, lista los archivos generados en `.dev/plan/` (incluyendo
   `parallel-plan.{json,md}`) y `.dev/features/` con un resumen de features, tareas,
   sprints, esfuerzo total y grado de paralelismo maximo.

...
2. Asegura el perfil de stack; resolve conmigo sus preguntas abiertas si las hay.
...
4. A medida que cada una termina, corre su `build-reviewer`; hallazgos high/medium
   rebotan al implementador de esa feature hasta pasar. Un bloqueo en una feature no
   frena a las demas.
5. Por cada feature que paso: push, PR contra la rama de integracion, limpieza del
   worktree y actualizacion de `progress.json`.
6. Al final, resumen por feature (tareas, veredicto, PR), bloqueos si los hubo, y el
   proximo paso (mergear PRs; cuando esten `done`, el siguiente lote).
